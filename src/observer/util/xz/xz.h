...
// void compress_file(const char *fname, const char *outName, int cLevel, int nbThreads)
// {
//     FILE* const fin  = fopen(fname, "rb");
//     FILE* const fout = fopen(outName, "wb+");
//     	// Get the preset number from the command line.
//     uint32_t preset = LZMA_PRESET_EXTREME;
...
// 	// stdin to stdout.
//     init_encoder(&strm, preset);
//     compress(&strm, fin, fout);
//     fclose(fin);
//     fclose(fout);
// }
...
// {
//     FILE* const fin  = fopen(fin_name, "rb");
//     FILE* const fout = fopen(fout_name, "wb+");

// 	lzma_stream strm = LZMA_STREAM_INIT;

//     init_decoder(&strm);
//     decompress(&strm, fin_name, fin, fout);
//     lzma_end(&strm);
