...
{
  return std::string(base_dir) + common::FILE_PATH_SPLIT_STR + table_name + TABLE_DATA_STR_SUFFIX;
}

std::string table_huf_file(const char *base_dir, const char *table_name)
{
  return std::string(base_dir) + common::FILE_PATH_SPLIT_STR + table_name + TABLE_HUF_SUFFIX;
}
